...
Fixpoint set_Rs (R : RegFile) (lv : list (RegName * Word)) :=
  match lv with
  | (rn, v) :: lv' =>
    set_Rs (RegMap.set rn v R) lv'
  | nil => R
  end.
...
| Ld_step : forall aexp (ri : GenReg) M R R' addr v,
    ...
    M addr = Some v -> RegMap.set ri v R = R' ->
    R__ (M, R) (ld aexp ri) (M, R')

| ST_step : forall (ri : GenReg) aexp M M' R addr v,
    eval_addrexp R aexp = Some addr -> word_aligned addr = true ->
    get_R R ri = v -> indom addr M -> MemMap.set addr (Some v) M = M' ->
...
| Add_step : forall M (R R' : RegFile) oexp (rs rd : GenReg) v1 v2,
    get_R R rs = v1 -> eval_opexp R oexp = Some v2 ->
    RegMap.set rd (v1 +ᵢ v2) R = R' ->
...
| Subcc_step : forall M (R R' : RegFile) oexp (rs rd : GenReg) v1 v2 v,
    get_R R rs = v1 -> eval_opexp R oexp = Some v2 -> v = v1 -ᵢ v2 ->
...
| Sll_step : forall M (R R' : RegFile) oexp (rs rd : GenReg) v1 v2,
    get_R R rs = v1 -> eval_opexp R oexp = Some v2 ->
    RegMap.set rd (v1 <<ᵢ (get_range 0 4 v2)) R = R' ->
...
| Rd_step : forall M (R R' : RegFile) (rsp : SpReg) (ri : GenReg) v,
    get_R R rsp = v -> RegMap.set ri v R = R' ->
    R__ (M, R) (rd rsp ri) (M, R')

| GetCwp_step : forall M (R R' : RegFile) (ri : GenReg) v,
    get_R R cwp = v -> RegMap.set ri v R = R' ->
...
| SSave :
    forall (M : Memory) (R R' R'': RegFile) D F F' k k' oexp
      ...
      get_R R cwp = k -> get_R R Rwim = v ->
      fetch R = (fmo, fml, fmi) ->
      get_R R rs = v1 -> eval_opexp R oexp = Some v2 -> F = F' ++ (fm1 :: fm2 :: nil) ->
      R' = replace fm1 fm2 fmo R -> k' = pre_cwp k -> win_masked k' v = false ->
...
| RRestore :
    forall (M : Memory) (R R' R'': RegFile) D F F' k k' oexp
      ...
      get_R R cwp = k -> get_R R Rwim = v ->
      fetch R = (fmo, fml, fmi) ->
      get_R R rs = v1 -> eval_opexp R oexp = Some v2 -> F = fm1 :: fm2 :: F' ->
      R' = replace fmi fm1 fm2 R -> k' = post_cwp k -> win_masked k' v = false ->
...
| Wr :
    forall M (R : RegFile) F D D' (rs : GenReg) (rsp : SpReg) oexp v1 v2 v,
      get_R R rs = v1 -> eval_opexp R oexp = Some v2 ->
      v = set_spec_reg rsp (v1 xor v2) -> D' = set_delay rsp v D ->
...
| Jumpl :
    forall C M aexp rd (R R' : RegFile) F D (pc npc f : Label),
      ...
      word_aligned f = true -> RegMap.set rd pc R = R' ->
      H__ C ((M, (R, F), D), pc, npc) ((M, (R', F), D), npc, f)

| Call :
    forall C M (R R' : RegFile) F D pc npc f,
      C pc = Some (ccall f) -> RegMap.set r15 pc R = R' ->
      H__ C ((M, (R, F), D), pc, npc) ((M, (R', F), D), npc, f)

| Retl :
    forall C M (R : RegFile) F D pc npc f,
      C pc = Some (cretl) -> get_R R r15 = f ->
      H__ C ((M, (R, F), D), pc, npc) ((M, (R, F), D), npc, f +ᵢ ($ 8))

| Be_true :
    forall C M (R : RegFile) F D pc npc f v,
      C pc = Some (cbe f) -> get_R R z = v -> v <> ($ 0) ->
      H__ C ((M, (R, F), D), pc, npc) ((M, (R, F), D), npc, f)

| Be_false :
    forall C M (R : RegFile) F D pc npc f,
      C pc = Some (cbe f) -> get_R R z = ($ 0) ->
      H__ C ((M, (R, F), D), pc, npc) ((M, (R, F), D), npc, npc +ᵢ ($ 4))

| Bne_true :
    forall C M (R : RegFile) F D pc npc f,
      C pc = Some (cbne f) -> get_R R z = ($ 0) ->
      H__ C ((M, (R, F), D), pc, npc) ((M, (R, F), D), npc, f)

| Bne_false :
    forall C M (R : RegFile) F D pc npc f v,
      C pc = Some (cbne f) -> get_R R z = v -> v <> ($ 0) ->
      H__ C ((M, (R, F), D), pc, npc) ((M, (R, F), D), npc, npc +ᵢ ($ 4)).
