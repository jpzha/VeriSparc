...
Definition changeY_pre (vl : list logicvar) :=
  EX fmo fml fmi fret x vy vi id fm1 fm2 F,
  [| vl = logic_fm fmi :: logic_fm fm1 :: logic_fm fm2
          :: logic_lv x :: logic_lv fret :: logic_lv vy :: logic_lv vi :: nil |] **
  Regs fmo fml fmi ** Ry |=> vy ** Rwim |=> vi ** {| id, fm1 :: fm2 :: F |} **
  [| get_frame_nth fmi 7 = Some fret /\ get_frame_nth fmi 0 = Some x
     /\ winvalid (post_cwp id) vi |].

Definition changeY_post (vl : list logicvar) :=
  EX fmo fml fmi fret x vy vi id fm1 fm2 F,
  [| vl = logic_fm fmi :: logic_fm fm1 :: logic_fm fm2
          :: logic_lv x :: logic_lv fret :: logic_lv vy :: logic_lv vi :: nil |] **
  Regs (update_frame fmi 0 vy) fm1 fm2 ** Ry |=> vy ** Ry |=> x ** Rwim |=> vi **
